...
def render_site_html(site_json: dict) -> str:
    ...
    generated_by = escape(site_json.get("_generatedBy", "unknown"))
    ai_model = escape(site_json.get("_aiModel", ""))
        ...
        for section in page.get("sections", []):
            ...
            if section_type == "hero":
                ...
                <div class="hero-block">
                    <span class="badge">Generated by HGGps · {generated_by} {ai_model}</span>
                    <h1>{title}</h1>
                    <p>{subtitle}</p>
...
<head>
    ...
    <style>
        * {{ margin: 0; padding: 0; box-sizing: border-box; }}
        ...
        body {{ font-family: Arial, sans-serif; background: #0b0b10; color: #ffffff; }}
        header {{ width: 100%; padding: 20px 8%; background: #12121a; border-bottom: 1px solid #2a2d3a; display: flex; align-items: center; justify-content: space-between; position: sticky; top: 0; z-index: 100; }}
        header strong {{ color: #e11d2e; font-size: 24px; }}
        nav {{ display: flex; gap: 18px; flex-wrap: wrap; }}
        nav a {{ color: #b8bcc8; text-decoration: none; font-size: 15px; }}
        nav a:hover {{ color: #e11d2e; }}
        .page-section {{ padding: 60px 8%; max-width: 1200px; margin: 0 auto; }}
        .page-section h2 {{ font-size: 34px; margin-bottom: 24px; }}
        .hero-block, .card {{ background: #12121a; border: 1px solid #2a2d3a; border-radius: 24px; padding: 34px; margin-bottom: 24px; box-shadow: 0 18px 40px rgba(0, 0, 0, 0.35); }}
        .badge {{ display: inline-block; padding: 8px 14px; background: rgba(225, 29, 46, 0.12); color: #e11d2e; border-radius: 999px; font-size: 14px; font-weight: 700; margin-bottom: 18px; }}
        .hero-block h1 {{ font-size: 46px; line-height: 1.1; margin-bottom: 16px; }}
        .hero-block p, .card p, .feature-item {{ color: #b8bcc8; line-height: 1.7; font-size: 16px; }}
        .hero-block button {{ margin-top: 22px; padding: 14px 22px; border: none; border-radius: 14px; background: #e11d2e; color: white; font-weight: 700; cursor: pointer; }}
        .hero-block button:hover {{ background: #b91523; }}
        .card h3 {{ font-size: 24px; margin-bottom: 16px; }}
        .features-grid {{ display: grid; grid-template-columns: repeat(3, 1fr); gap: 14px; }}
        .feature-item {{ background: #191923; border: 1px solid #2a2d3a; border-radius: 16px; padding: 18px; }}
